...
| Commit | Change |
|---|---|
| (2026-08-15 session) | **Outcome tracking unblocked + governance loop closed.** (1) **tz-aware index broke every historical price lookup.** yfinance returns a tz-AWARE index (Asia/Kolkata) for NSE symbols; `_fetch_price_on_date` built tz-naive Timestamps from `rec_date`, so the subtraction raised `TypeError: Cannot subtract tz-naive and tz-aware datetime-like objects` — swallowed by the except clause into a silent `None` **at DEBUG level**. Every `nifty_entry` ever written was therefore NULL, and both `run_outcome_tracking()` and `run_forward_polling()` skip such rows on `if not entry_price or not nifty_entry: continue`. That single field froze the whole Performance tab for ~3 months. Fixed in `agents/outcome_tracker.py` and `agents/paper_portfolio.py` (tz stripped before any date maths); both now log failures at **WARNING**, which immediately exposed a *second* masked bug in paper_portfolio — `index - Timestamp` yields a `TimedeltaIndex` with no `.abs()` in this pandas version, so that lookup also returned None every call. `scripts/backfill_nifty_entry.py` repaired the 148 existing rows (62 NIFTY lookups, one per distinct rec_date). Poller then reported `polled=148 live_updated=148 t30_resolved=119 errors=0`; `outcome_t30` went 148 PENDING → 40 HIT / 66 PARTIAL / 13 MISS / 29 not-yet-due. t+90 correctly still empty (oldest rec 2026-05-18 → first resolution ~2026-08-16). (2) **Governance had a producer with no consumer.** `approve_proposal()` was reachable only from the CLI — no worker job, no API endpoint; `dashboard/api/research.js` wrote a status field under a comment claiming it "triggers Python approve_proposal via note", which nothing watches. 20 proposals and 160 debate entries had accumulated, including one at 8 FOR / 0 AGAINST still `pending`. New `evaluate_auto_approval()` + `auto_approve_proposals()` (supermajority ≥75% FOR, ≤1 AGAINST, ≥5 decisive votes, `cost_impact` in low/none/free/zero, `metadata.requires_paid_data` falsy, 3 PRs/run) wired after the 07:30 scan. Auto-**merge** added on top: `evaluate_auto_merge()` + `auto_merge_approved_prs()`, **unanimous only**, CI fully green, opt-in via `GOVERNANCE_AUTO_MERGE=true`, 2 merges/run. New `github_manager.get_pr_checks()` reads the **check-runs** API (GitHub Actions does not populate the legacy commit-status API, so a combined-status query returns empty and reads as "nothing failed") and treats **zero checks as not-green** so an untested PR fails closed. (3) `get_system_health._ok()` took 2 args but the Daily Pipeline check assigns `check_fn = _ok | _warn` and calls it with a hint — so the health panel crashed **only when the pipeline succeeded**, and the try/except reframed it as "Verify Supabase schema has daily_runs table". `_ok` now accepts an optional action. (4) `scripts/diagnose_performance.py` — verifies the key is `service_role` before reading (an anon key returns 0 rows under RLS and looks identical to an empty database) and reports which of the three failure modes is blocking. 76 new tests. |
| (2026-08-14 session) | **ARIA /analyse "no recommendation" — two root causes, both fixed.** Railway log 2026-08-13 19:35: `[TATASTEEL.NS] synthesis failed: float() argument must be a string or a real number, not 'NoneType'`. (1) **Claude JSON nulls crashed `_build_recommendation()`.** Claude emits explicit nulls for numeric fields that don't apply to the call it made (`"target": null`, `"upside_pct": null` on a HOLD — confirmed live on INFY). The orchestrator read them as `float(synthesis_data.get(key, default))`, and `dict.get(key, default)` does **not** substitute the default when the key exists with a None value → `TypeError`, symbol dropped from the run. New `_num(value, default)` helper in `scheduler/orchestrator.py` coerces None/NaN/inf/unparseable to a default; every numeric read of `synthesis_data` now goes through `_num()` (or `_f()` for the nullable price levels, which must stay None rather than collapse to 0.0). Sites fixed: `_build_recommendation` confidence/risk_score/horizon_days/upside_pct/upside_confidence/danger_drop_pct/danger_confidence, `_apply_consensus_gate`, the two P7-C confidence caps, and the earnings-guard downgrade. Also derives `upside_pct` from `target` vs current price when Claude gives a target but a null upside. (2) **`get_ohlcv()` returned yfinance's partial bar.** yfinance 1.2.x appends a current-session row with Volume but NaN OHLC; `close.iloc[-1]` was therefore NaN, so `DataCompletenessValidator` failed on "Current close price" and the **technical agent returned INSUFFICIENT_DATA for every symbol** (visible in the log as `completeness=88% critical_below_threshold=[Current close price]`). `data/fetchers.py::get_ohlcv` now drops NaN-Close rows once, for all callers — TATASTEEL technical went INSUFFICIENT_DATA → BUY(62), GAIL → STRONG_BUY. Same NaN-bar guard added to `regime_detector._fetch_ohlcv`, `outcome_tracker._price_on_date`, `paper_portfolio._price_on_date` and `target_updater._quick_technicals` (RSI). (3) `POST /api/analyse` now returns the real `state["errors"]` in the NO_RECOMMENDATION response instead of always claiming "suppressed", so a crash is distinguishable from a genuine validation suppression on the dashboard. (4) `db/migrations/allow_suppressed_action.sql` (✅ applied 2026-08-14) — the `recommendations_action_check` constraint rejected `action='SUPPRESSED'` (log 19:14:53, error 23514), so `_log_suppressed_synthesis()` had never persisted a row; migration widens the constraint, and the code now names the migration in its warning. 25 new tests in `tests/test_synthesis_null_fields.py`. Verified end-to-end locally (dry-run pipeline) on TATASTEEL/GAIL/INFY — all three return a full recommendation, `errors=[]`. |
| (2026-08-03 session — part 2) | **Discovery card UX overhaul** (commit `579edfd`): (1) `agents/discovery_screener.py` — `_save_discovery()` now computes and persists `entry_low` (price×0.98), `entry_high` (price×1.01), `target` (price×(1+upside/100)), `stoploss` (ATR floor from technical agent, fallback 15% below price) — was hardcoded None, so all discovery cards showed "—" for entry/target/stop; applied to both INSERT and UPDATE paths. (2) `api/main.py` — `_transform_recommendation()` gains `opportunityTier` field (CRITICAL/STANDARD from headline prefix). (3) `dashboard/src/App.jsx` — `ConfidenceGauge` component (confidence → green arc, replaces `RiskGauge` on discovery cards; riskScore=0 froze all gauges at "0 Low"); `BULL_SIGNALS`/`BEAR_SIGNALS` sets added (fixes 0 bull count — STRONG_BUY, BULLISH_ANALOGUE weren't in old array); `sc()` signal color in deep-dive updated same way; dynamic agent count label in deep-dive ("N-Agent Analysis"); `DiscoveryRecsTable` — new paginated (30/page) consolidated table of all live discovery recs sorted by date desc with entry/target/stoploss; ARIA analyse error now shows friendly NSE ticker hint on 4xx. **Known open issues from screenshots**: (a) Governance "24 symbols 24 errors" daily — likely kappa suppression or synthesis failure for all portfolio symbols; requires Railway logs to confirm root cause. (b) Fundamental agent hallucination rate high (DEGRADING trend) — data gap when screener.in returns 403 may cause agent to produce claims that fact_checker contradicts. Both need Railway log investigation. |
...
| ARIA /analyse returns NO_RECOMMENDATION — `float(None)` on Claude's JSON nulls kills the symbol | CRITICAL | `scheduler/orchestrator.py` | ✅ Fixed 2026-08-14 — `_num()` helper; every `synthesis_data` numeric read is None-safe |
| Technical agent returns INSUFFICIENT_DATA for every symbol — yfinance partial bar has NaN Close | CRITICAL | `data/fetchers.py` `get_ohlcv` | ✅ Fixed 2026-08-14 — NaN-Close rows dropped at the fetcher; same guard added to regime_detector / outcome_tracker / paper_portfolio / target_updater |
| `/api/analyse` reports every failure as "suppressed", hiding real crashes | MEDIUM | `api/main.py` | ✅ Fixed 2026-08-14 — `state["errors"]` surfaced in the NO_RECOMMENDATION response |
| `action='SUPPRESSED'` violates `recommendations_action_check` (23514) — suppressed recs never persisted | MEDIUM | `db/schema.sql`, `scheduler/orchestrator.py` | ✅ Fixed 2026-08-14 — `db/migrations/allow_suppressed_action.sql` applied in Supabase |
