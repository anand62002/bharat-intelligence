...
>
> **⏳ Pending:** `db/migrations/allow_suppressed_action.sql` — widens `recommendations_action_check` to accept `action='SUPPRESSED'`. Until it is run in Supabase → SQL Editor, `_log_suppressed_synthesis()` fails with error 23514 on every suppressed recommendation and those rows are lost for human review.
...
| `action='SUPPRESSED'` violates `recommendations_action_check` (23514) — suppressed recs never persisted | MEDIUM | `db/schema.sql`, `scheduler/orchestrator.py` | 🔲 Run `db/migrations/allow_suppressed_action.sql` in Supabase |
