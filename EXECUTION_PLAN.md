...
| P7-F | **Partial/Degraded Data Alerting** — Add a `data_quality_summary` to each daily run: (1) per-symbol flag for which data source was used (screener full / screener partial / trendlyne fallback / yfinance only / NO_DATA). (2) Alert via portfolio_alerts when > 30% of symbols in a run used degraded data. (3) Dashboard Governance tab: "Data Source Quality" panel showing per-run data source breakdown. (4) Synthesis prompt: inject explicit data quality preamble — "WARNING: 3 of 7 agents used partial data for this symbol. Treat specific numerical claims with appropriate uncertainty." This ensures Fable 5/Sonnet sees data quality context before reasoning. | Phase 7 | ✅ DONE 2026-08-04 — FULL/FALLBACK/ESTIMATED/UNKNOWN counted per run; DATA_QUALITY_DEGRADED alert above 30%; stored in `daily_runs.agents_run`; `GET /api/governance/data-quality` + `DataSourceQualityPanel` in Governance tab | — |
| P7-H | **Fundamentals Cache — Phase 1: intra-run memoisation** — `fundamental`, `warren_bot`, `mgmt_quality`, `governance_screener` and `insider_signal` (called twice: once by sentiment, once by institutional) each independently call `get_screener_data`/`get_screener_history` for the *same* symbol inside a single run — roughly 9 remote fetches per stock where 2 would do. Fix: a request/run-scoped memo cache in `data/fetchers.py` keyed by (symbol, fn), cleared at run start. Zero staleness risk — it returns exactly the data the same run would otherwise have refetched seconds later. Expected: ~640 → ~150 remote fetches/day, and a matching drop in the 403/rate-limit pressure that pushes the fundamental agent onto degraded yfinance data. No migration, no schema change. | Phase 7 | ✅ DONE 2026-08-04 — `data/run_cache.py`: per-key-locked memoisation with double-checked reads (concurrent duplicates collapse to one fetch, which matters because fundamental/warren_bot/mgmt_quality run in the same `asyncio.gather`); applied to `get_screener_data` + `get_screener_history`; scoped per run in `run_pipeline` and `run_discovery`; saving surfaced on `/api/system/health`. Measured 9 → 2 fetches per symbol (77.8% avoided). 17 tests. | — |
| P7-I | **Fundamentals Cache — Phase 2: persistent cache with decay** — Fundamentals change quarterly; we refetch them daily. Add a `fundamentals_cache` table (symbol, payload JSONB, fetched_at, source, ttl_class) with TTL driven by what the field actually is rather than one flat number: snapshot ratios/PE ~7d, 10-yr annual history ~30d, promoter holding ~30d (quarterly filings). Force a refresh when a symbol's `earnings_calendar` date passes — the one event that genuinely invalidates fundamentals. On a source outage, serve stale-but-flagged data (`data_quality="CACHED_STALE"`, age surfaced in the agent rationale) instead of silently degrading to yfinance, which *improves* quality on blocked days. Precondition for P7-G: Trendlyne MCP allows 2,000 calls/month vs our current ~19,000 — the cache is what makes the MCP option viable at all. | Phase 7 | ✅ DONE 2026-09-09 — `data/fundamentals_cache.py` + `db/migrations/create_fundamentals_cache.sql`. Per-kind TTL (snapshot 7d, history 30d), early invalidation when `earnings_calendar` shows results since the fetch, and a quality rank (screener 3 / trendlyne 2 / yfinance 1) so a degraded fetch can never overwrite a good cached row **and** an outage serves last-known-good real fundamentals instead of thin yfinance data. Layered beneath the P7-H memo. Never raises. 25 tests. **Migration must be run in Supabase.** | — |

**Progress: 78 / 90 items complete (87%) — Session 2026-08-04: P7-C/P7-D/P7-F ✅ shipped; P7-H + P7-I (fundamentals cache, Phase 1 → Phase 2) added (+2 items)**

> **Next up — fundamentals caching, in this order:** P7-H (intra-run memoisation) then P7-I (persistent cache with decay).
> Motivation, from call-site analysis: ~640 remote screener.in/Trendlyne fetches per day (~19,000/month) for data that changes quarterly.
> Orchestrator ≈ 24 symbols × ~9 fetches ≈ 215/day; discovery ≈ 200 pre-screen + 25 deep × ~9 ≈ 425/day. There is no cache at any layer today
> (`screener_snapshot` in the codebase is only a source *label*, not a table). This volume from a single Railway IP is the likely driver of the
> fundamental agent's DEGRADING trend, and it puts us ~10× over the Trendlyne MCP Pro allowance of 2,000 calls/month.
...
| BF-26 | **Suppressed recs never persisted** — `action='SUPPRESSED'` violates `recommendations_action_check` (error 23514, logged 19:14:53). `db/migrations/allow_suppressed_action.sql` widens the constraint; orchestrator now names the migration in its warning. | ✅ DONE (migration applied in Supabase) | 2026-08-14 |

| BF-27 | **tz-aware index NULLed every `nifty_entry`** — yfinance returns a tz-AWARE index (Asia/Kolkata) for NSE symbols; `_fetch_price_on_date` built tz-naive Timestamps from `rec_date`, so the subtraction raised `TypeError` which the except clause swallowed into a silent `None` **at DEBUG level**. Both the outcome tracker and the forward poller skip rows on `if not entry_price or not nifty_entry: continue`, so all 148 rows were skipped and the entire Performance tab sat empty for ~3 months. Fixed in `outcome_tracker.py` + `paper_portfolio.py`; failures now log at WARNING. | ✅ DONE | 2026-08-15 |
| BF-28 | **Second, masked bug in `paper_portfolio._fetch_price_on_date`** — `index - Timestamp` yields a `TimedeltaIndex`, which has no `.abs()` in this pandas version, so the lookup returned None on every call. Invisible until BF-27's log-level change surfaced it. Built as a Series instead. | ✅ DONE | 2026-08-15 |
| BF-29 | **Health panel crashed only on success** — `get_system_health` assigns `check_fn = _ok | _warn` then calls it with a hint argument, but `_ok` took 2 args. The success path raised, and the try/except reported it as "Could not read daily_runs — Verify Supabase schema", pointing at a non-existent schema fault. `_ok` now takes an optional action. | ✅ DONE | 2026-08-15 |
| BF-30 | **148 stranded outcome rows backfilled** — `scripts/backfill_nifty_entry.py` (62 NIFTY lookups for 148 rows). Poller: `polled=148 live_updated=148 t30_resolved=119 errors=0`; `outcome_t30` 148 PENDING → 40 HIT / 66 PARTIAL / 13 MISS / 29 not-yet-due. t+90 correctly empty until ~2026-08-16. | ✅ DONE | 2026-08-15 |
| GOV-1 | **Governance loop closed** — `approve_proposal()` had no automated caller (CLI only; `research.js` wrote a status field under a comment claiming it triggered the Python function, which nothing watches), so 20 proposals and 160 debate entries accumulated with a unanimous 8-FOR proposal still `pending`. New supermajority gate (≥75% FOR, ≤1 AGAINST, ≥5 votes, zero cost impact, no paid data, 3 PRs/run) runs after the 07:30 scan. Preview: `python governance/research_agent.py --auto-approve --dry`. | ✅ DONE | 2026-08-15 |
| GOV-2 | **Auto-merge** — unanimous vote + fully green CI, opt-in via `GOVERNANCE_AUTO_MERGE=true`, 2 merges/run. New `github_manager.get_pr_checks()` reads the check-runs API (GitHub Actions does not populate the legacy commit-status API, so a combined-status query returns empty and reads as "nothing failed") and treats zero checks as not-green, so an untested PR fails closed. Merges land on the run *after* the PR, since CI needs minutes. | ✅ DONE | 2026-08-15 |
| OPS-3 | **`scripts/diagnose_performance.py`** — verifies the key is `service_role` before reading, since an anon key returns 0 rows under RLS and is indistinguishable from an empty database. Reports NULL-field skips, poller status and what is genuinely due. | ✅ DONE | 2026-08-15 |
...
| **GOV-3** | **Research-PR consumption strategy — the queue does not converge.** As of 2026-09-09 the gate reported `checked=381 approved=3 skipped=378`. The research agent creates ~17 proposals/day; auto-approval opens at most 3 PRs/run, so the backlog *grows* ~14/day and 99% of what the gate evaluates is stale. Decide: (a) where PRs land and who/what reviews them — GitHub review, a dashboard queue, or ARIA; (b) raise the per-run cap and/or run the gate more often; (c) tighten the relevance bar at *creation* so fewer proposals are minted; (d) bulk-reject or expire proposals older than N days; (e) dedupe — many are near-identical papers re-proposed. Must be settled BEFORE enabling `GOVERNANCE_AUTO_MERGE`, otherwise merges are drawn from a queue that is almost entirely backlog. | Code + Decision | None | M | ⬜ TODO |
...
| **P7-I** | Fundamentals cache Phase 2 — persistent cache with per-field decay (ratios ~7d, annual history ~30d, promoter ~30d); earnings-date invalidation; serve stale-but-flagged on source outage. Precondition for P7-G | Code | None | M | ✅ Done 2026-09-09 |
